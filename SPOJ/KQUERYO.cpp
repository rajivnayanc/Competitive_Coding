...
class SegmentTree {
    vector<ll> vect[4*MAX+1];
    public:
    void build(ll idx, ll start, ll end){
        if(start==end){
            vect[idx].push_back(arr[start]);
        }else{
            ll mid = (start+end)/2;
            ...
            merge(vect[l_i].begin(),vect[l_i].end(),
                    vect[r_i].begin(),vect[r_i].end(), 
                    back_inserter(vect[idx]));

        }
    }

    ll query(ll idx, ll ss, ll se, ll qs, ll qe, ll k){
        if(qs>se || qe<ss) return 0;
        if(qs<=ss && qe>=se){
            ll t1 = upper_bound(all(vect[idx]),k) - vect[idx].begin();
            ll temp = (ll)vect[idx].size()-t1;
            return temp;
        }

        ll mid = (se+ss)/2;
        ll leftQuery = query(2*idx+1,ss,mid,qs,qe,k);
        ll rightQuery = query(2*idx+2,mid+1,se,qs,qe,k);
...
void solve(){
    ...
    while(Q--){
        ...
        k = ans^c;
        if(i>j) ans=0;
        else ans = segTree->query(0,0,N-1,i-1,j-1,k);
        cout<<ans<<"\n";
    }
}
int main(){
