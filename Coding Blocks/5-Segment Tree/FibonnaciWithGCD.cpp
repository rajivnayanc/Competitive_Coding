...
class SegmentTree {
    ...
    void build(ll idx, ll start, ll end){
        if(start==end){
            ...
            vect[idx] = gcd(vect[l_i],vect[r_i]);
        }
    }
...
vector<vector<ll>> matPowFast(vector<vector<ll>> A, ll y){
    ll R = A.size();
    ll C = A[0].size();

    vector<vector<ll>> result(R,vector<ll>(C,0));
    for(ll i=0;i<R;i++) result[i][i]=1;

    while(y>0){
        if(y&1) result = matMul(result,A);
        A = matMul(A,A);
        y/=2;
    }
    return result;
...
void solve(){
    ...
    while(Q--){
        cin>>L>>R;
        ans = segTree->query(0,0,N-1,L-1,R-1);
        cout<<computeFibonnaci(ans)<<endl;
    }
